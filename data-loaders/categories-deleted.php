...
for ($currentPage = $options['page']; $currentPage <= $maxPages; $currentPage++) {
    ...
    try {
        ...
        $results = $tevo->listCategoriesDeleted($options);
    } catch(Exception $e) {
        /**
    ...
    foreach ($results AS $result) {
        ...
        if (!$row->save()) {
            echo '<h1 class="error">'
               . htmlentities('Error attempting to ' . $action . ' ' . $result->id . ': ' . $result->name . ' as deleted to `tevoCategories`', ENT_QUOTES, 'UTF-8', false)
               . '</h1>' . PHP_EOL
            ;
        } else {
            echo '<h1>'
               . htmlentities('Successful ' . $action . ' of ' . $result->id . ': ' . $result->name . ' as deleted to `tevoCategories`', ENT_QUOTES, 'UTF-8', false)
               . '</h1>' . PHP_EOL
            ;
        }
        unset($action);
