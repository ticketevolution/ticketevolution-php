...
for($currentPage = $options['page']; $currentPage <= $maxPages; $currentPage++) {
    ...
    foreach($results AS $result) {
        ...
        if(!$row->save()) {
            echo '<h1 class="error">Error attempting to save ' . htmlentities($data['performerId'] . ': ' . $data['performerName']) . ' to `tevoPerformers`</h1>' . PHP_EOL;
        } else {
            echo '<h1>Saved ' . htmlentities($data['performerId'] . ': ' . $data['performerName']) . ' to `tevoPerformers`</h1>' . PHP_EOL;
        }
        unset($data);
        unset($row);
